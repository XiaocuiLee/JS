...
function top2(){
   	form3.action="<%=basePath%>admin/yy/yysinfo.jsp?page=1";
    form3.submit();
}
function last2(){
	...
	form3.action="<%=basePath%>admin/yy/yysinfo.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre2(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%>admin/yy/yysinfo.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump2(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%>admin/yy/yysinfo.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%>admin/yy/yysinfo.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%>admin/yy/yysinfo.jsp?page="+page;
      form3.submit();
...
<%
	...
	int cou = cb.getMessageCount("select count(*) from prep where member='"+user+"' and yybh like '%"+word+"%'");//得到信息总数			        
	...
	List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from prep where member='"+user+"' and yybh like '%"+word+"%' order by id desc",14);
...
%>       
       <tr align="center">
         ...
         <td nowrap="nowrap"><a href="<%=basePath%>ComServlet?method=delyy&id=<%=pagelist2.get(0).toString()%>">删除</a></td>
       </tr>
<% }} %>
       <tr align="center">
