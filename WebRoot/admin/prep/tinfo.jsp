...
function top2(){
   	form3.action="<%=basePath%>admin/prep/tinfo.jsp?page=1";
    form3.submit();
}
function last2(){
	...
	form3.action="<%=basePath%>admin/prep/tinfo.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre2(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%>admin/prep/tinfo.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump2(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%>admin/prep/tinfo.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%>admin/prep/tinfo.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%>admin/prep/tinfo.jsp?page="+page;
      form3.submit();
...
<div class="title_right"><strong>使用信息统计</strong></div>  
...
<%
	...
	int cou = cb.getMessageCount("select count(*) from prep where mc like '%"+mc+"%' and sj>='"+ksj+"' and sj<='"+esj+"' and sh='通过'");//得到信息总数			        
	...
	List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from prep where mc like '%"+mc+"%' and sj>='"+ksj+"' and sj<='"+esj+"' and sh='通过' order by id desc",14);
...
<% }} %>
       <tr align="center">
         <td nowrap="nowrap" colspan="14">共： <%=cou %> 次<input type="hidden" name="mc" value="<%=mc%>" /><input type="hidden" name="ksj" value="<%=ksj%>" /><input type="hidden" name="esj" value="<%=esj%>" />
