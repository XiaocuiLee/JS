...
function top2(){
   	form3.action="<%=basePath%>admin/room/sinfo.jsp?page=1";
    form3.submit();
}
function last2(){
	...
	form3.action="<%=basePath%>admin/room/sinfo.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre2(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%>admin/room/sinfo.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump2(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%>admin/room/sinfo.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%>admin/room/sinfo.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%>admin/room/sinfo.jsp?page="+page;
      form3.submit();
...
<table class="table table-bordered table-striped table-hover">
     <tbody>
       <tr align="center">
         ...
         <td nowrap="nowrap"><strong>教室设备</strong></td> 
         <td nowrap="nowrap"><strong>教室介绍</strong></td> 
         <td nowrap="nowrap"><strong>状态</strong></td> 
         <td nowrap="nowrap"><strong>借用信息</strong></td> 
         <td width="80" nowrap="nowrap"><strong>操作</strong></td>
       </tr>
<%  String word=Common.toChineseAndTrim(request.getParameter("word"));
	cb.setEVERYPAGENUM(12);
	int cou = cb.getMessageCount("select count(*) from room where mc like '%"+word+"%'");//得到信息总数			        
	...
	List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from room where mc like '%"+word+"%' order by id desc",7);
...
%>       
       <tr align="center">
         ...
         <td nowrap="nowrap"><a href="<%=basePath%>admin/room/yyinfo.jsp?roomid=<%=pagelist2.get(0).toString()%>">查看</a></td>
         <td nowrap="nowrap">
         <a href="<%=basePath%>admin/room/zt.jsp?method=ztroom&id=<%=pagelist2.get(0).toString()%>">状态</a>
         <a href="<%=basePath%>admin/room/add.jsp?method=uproom&id=<%=pagelist2.get(0).toString()%>">修改</a>  
         <a href="<%=basePath%>ComServlet?method=delroom&id=<%=pagelist2.get(0).toString()%>">删除</a>
         </td>
       </tr>
<% }} %>
       <tr align="center">
